...
# Uses the official GreenHat hat design with tier colors.
#
...
B64() { echo -n "$1" | base64 -w0 | tr -d '\n'; }

JsonUri() {
    local NAME="$1" DESC="$2" COLOR="$3" SHADOW="$4"
    local SVG=$(SedSvg "$COLOR" "$SHADOW")
    local SVG_B64=$(B64 "$SVG")
    local JSON='{"name":"'"$NAME"'","description":"'"$DESC"'","image":"data:image/svg+xml;base64,'"$SVG_B64"'","attributes":[{"trait_type":"Tier","value":"'"$NAME"'"}]}'
    echo "data:application/json;base64,$(B64 "$JSON")"
}

SedSvg() {
    local COLOR="$1" SHADOW="$2"
    # The official GreenHat SVG with tier color substitution
    sed -e "s/{COLOR}/$COLOR/g" -e "s/{SHADOW}/$SHADOW/g" << 'SVG'
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="none" version="1.1" width="1942" height="1474" viewBox="0 0 1942 1474">
  <defs>
    <clipPath id="master_svg0_10_08"><rect x="0" y="0" width="1942" height="1474" rx="0"/></clipPath>
  </defs>
...
# ── Tier 1: Bronze ──
echo "  Setting Bronze..."
cast send "$NFT" "setTierURI(uint8,string)" 1 "$(JsonUri 'Bronze GreenHat' 'A humble bronze hat for GREEN believers.' '#CD7F32' '#8B5E3C')" --rpc-url "$RPC" --private-key "$PK" > /dev/null

# ── Tier 2: Silver ──
echo "  Setting Silver..."
cast send "$NFT" "setTierURI(uint8,string)" 2 "$(JsonUri 'Silver GreenHat' 'A shiny silver hat for dedicated GREEN holders.' '#C0C0C0' '#707070')" --rpc-url "$RPC" --private-key "$PK" > /dev/null

# ── Tier 3: Gold ──
echo "  Setting Gold..."
cast send "$NFT" "setTierURI(uint8,string)" 3 "$(JsonUri 'Gold GreenHat' 'A glorious gold hat for elite GREEN supporters.' '#FFD700' '#B8860B')" --rpc-url "$RPC" --private-key "$PK" > /dev/null

# ── Tier 4: Diamond ──
echo "  Setting Diamond..."
cast send "$NFT" "setTierURI(uint8,string)" 4 "$(JsonUri 'Diamond GreenHat' 'The legendary diamond hat. Only the truest GREEN believers.' '#B9F2FF' '#008B8B')" --rpc-url "$RPC" --private-key "$PK" > /dev/null
